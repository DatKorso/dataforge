...
with adv_cols[1]:
    filter_brand_at_end = st.checkbox(
        ...
        value=True,
        help="Применить фильтр бренда после объединения/дописывания. Рекомендуется для корректной работы с видео-листами.",
        key="filter_brand_at_end",
        disabled=not enable_brand_filter,
    )

st.info(
    "💡 **Рекомендация**: используйте \"Фильтр бренда в конце\" для корректной фильтрации всех листов, "
    "включая видео и видеообложку."
)
...
if st.button("🚀 Объединить файлы", type="primary", use_container_width=True):
    try:
        ...
        with st.expander("ℹ️ Важная информация о результате", expanded=False):
            st.markdown("""
            **Возможные причины неполного объединения:**
            
            - **Повреждённые листы Excel**: Если лист содержит повреждённые XML-данные, он будет пропущен
            - **Отсутствие листов**: Дополнительные файлы могут не содержать все листы из шаблона
            - **Фильтры удалили все данные**: Проверьте настройки фильтров по бренду и артикулам
            
            **Что проверить:**
            
            1. Откройте скачанный файл и проверьте наличие данных на всех листах
            2. Проверьте терминал/логи приложения на предупреждения (⚠️ WARNING/ERROR)
            3. Если лист был пропущен из-за повреждения, попробуйте:
               - Открыть исходный файл в Excel и пересохранить его
               - Скопировать данные в новую книгу Excel
               - Использовать "Сохранить как" → выбрать строгий формат .xlsx
            
            **Проверенные листы:** {included_sheets}
            """.format(included_sheets=", ".join([f"`{name}`" for name in included_sheets])))
